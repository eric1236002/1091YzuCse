...
public:
    ...
    void put()
    {
        ...
        for (size_t i = 0; i < data.size() && c; ++i) for (size_t j = 0; j < data[i].length() && c; ++j)
        {
            switch (data[i][j])
            {
            case '{':
            ...
            case '}':
                ...
                if (!islist.top()) nodes.top()->v.push_back(tmp);
                else nodes.top()->l.push_back(tmp);
                
                str.clear();
                if (nodes.size() == 1) c = false;
                break;
            case '[':
                valtmp->islist = true;
                nodes.push(valtmp);
    ...
    {
        if (r->key == str && r->islist) for (auto& p1 : r->l) print2(p1);
        else
        {
            if (r->key == str) cout << r->val << endl;
    ...
    {
        if (r->key.empty()) cout << r->val << endl;
        else
        {
